/* ===========================================================
   FIBEL — „Litera B cu Herzchen" · Claudia Toth © 2026
   Abecedar german interactiv pentru copii 4–8 ani.
   Temă: Bär 🐻 · „B wie Bär" · „Herzchen umarmt den Bären".

   17 secțiuni: Start · Litera B · Suche B · Cuvinte · Citește (cuvinte+propoziții) ·
   Legare silabe (unește) · Despărțire în silabe (clap) · Joc B · Wo hörst du B ·
...
const B = {
  baer:   {de:'der Bär',    icon:'baer',   audio:'b-baer', audioArt:'b-art-baer',   gen:'m', art:'der', ro:'ursul',    m:true},
  blume:  {de:'die Blume',  icon:'blume',  audio:'b-blume', audioArt:'b-art-blume',  gen:'f', art:'die', ro:'floarea',  m:true},
  buch:   {de:'das Buch',   icon:'buch',   audio:'b-buch', audioArt:'b-art-buch',   gen:'n', art:'das', ro:'cartea',   m:true},
  ball:   {de:'der Ball',   icon:'ball',   audio:'b-ball', audioArt:'b-art-ball',   gen:'m', art:'der', ro:'mingea',   m:true},
  banane: {de:'die Banane', icon:'banane', audio:'b-banane', audioArt:'b-art-banane', gen:'f', art:'die', ro:'banana',   m:true},
  bett:   {de:'das Bett',   icon:'bett',   audio:'b-bett', audioArt:'b-art-bett',   gen:'n', art:'das', ro:'patul',    m:true},
  maus:  {de:'die Maus',  icon:'m-maus',audio:'b-maus',  gen:'f', art:'die', ro:'șoricelul', m:false},
  mond:  {de:'der Mond',  icon:'m-mond',audio:'b-mond',  gen:'m', art:'der', ro:'luna',      m:false},
  katze: {de:'die Katze', icon:'katze', audio:'b-katze', gen:'f', art:'die', ro:'pisica',    m:false},
  apfel: {de:'der Apfel', icon:'apfel', audio:'b-apfel', gen:'m', art:'der', ro:'mărul',     m:false},
  obst:  {de:'das Obst',  icon:'obst',  audio:'b-obst',  gen:'n', art:'das', ro:'fructele',  m:false},
  tiger: {de:'der Tiger', icon:'tiger', audio:'b-tiger', gen:'m', art:'der', ro:'tigrul',    m:false},
};
const WORDS_B  = ['baer','blume','buch','ball','banane','bett'];
const GAME_POOL= ['baer','maus','blume','katze','ball','apfel'];
const ART_QUIZ = ['baer','blume','buch','bett'];
const GRID = [
  ['B','m','b','N','w'],['l','b','M','B','n'],['B','o','b','d','B'],['t','B','a','b','F'],['d','b','M','B','u'],
];
/* despărțire în silabe (clap) — 1 → 2 → 3 silabe */
const SPLIT = [
  {icon:'ball',   audio:'b-ball',   syl:['Ball'],          sylA:['b-syl-ball']},
  {icon:'blume',  audio:'b-blume',  syl:['Blu','me'],      sylA:['b-syl-blu','b-syl-me']},
  {icon:'banane', audio:'b-banane', syl:['Ba','na','ne'],  sylA:['b-syl-ba','b-syl-na','b-syl-ne']},
];
const STORY = [
  {de:'Das ist <b class="m-bold">Herzchen</b>.',                                ro:'Acesta e Herzchen (inimioara).', icon:'herzchen', audio:'st-1'},
  {de:'<b class="m-bold">Herzchen</b> sieht einen <b class="m-bold">Bären</b>.', ro:'Herzchen vede un urs.',          icon:'baer',     audio:'st-2'},
  {de:'<b class="m-bold">Herzchen</b> umarmt den <b class="m-bold">Bären</b>.',  ro:'Herzchen îmbrățișează ursul.',   icon:'baer',     audio:'st-3'},
  {de:'<b class="m-bold">Herzchen</b> ist glücklich.',                          ro:'Herzchen e fericit.',            icon:'herzchen', audio:'st-4'},
];
const QUIZ = [
  {type:'audio',   ans:'baer',   opts:['baer','maus','katze']},
  {type:'findm',   ans:'blume',  opts:['blume','katze','mond']},
  {type:'article', word:'buch'},
  {type:'audio',   ans:'banane', opts:['banane','apfel','tiger']},
  {type:'findm',   ans:'ball',   opts:['ball','obst','mond']},
];
const LETTER = 'B';
const POS = [
  {icon:'baer',  audio:'b-baer',  pos:'a'},
  {icon:'blume', audio:'b-blume', pos:'a'},
  {icon:'buch',  audio:'b-buch',  pos:'a'},
  {icon:'ball',  audio:'b-ball',  pos:'a'},
];
...
SCREENS[0] = function(){
  ...
  show(
    ...
    '<h1 class="title">Litera B cu Herzchen</h1>' +
  ...
  document.getElementById('startBtn').onclick = ()=>{ play('b-wie-baer'); go(1); };
...
SCREENS[1] = function(){
  show(
    ...
    '<div class="letter-hero">B<span class="lo">b</span></div>' +
    '<div class="btn-row">' +
      '<button class="big-btn" id="bName">🔊 B</button>' +
      '<button class="big-btn amber" id="bMaus">🔊 B wie Bär</button>' +
    '</div>' +
    '<p class="parent-note">Atinge: „B" — litera se citește „be" · „B wie Bär" = B ca în Bär (urs).</p>' +
    nextBtn()
  );
  document.getElementById('bName').onclick = ()=>play('b-name');
  document.getElementById('bMaus').onclick = ()=>play('b-wie-baer');
  setTimeout(()=>play('b-wie-baer'), 350);
  wireNext(2);
};
...
SCREENS[2] = function(){
  ...
  GRID.forEach(row=> row.forEach(ch=>{
    const isM = (ch==='B'||ch==='b'); if(isM) total++;
    cells += '<div class="lcell" data-m="'+(isM?1:0)+'">'+ch+'</div>';
  }));
  show(
    '<p class="kicker">Suche B und b 🔍</p>' +
    '<h1 class="title" style="font-size:1.4rem">Atinge toți B și b</h1>' +
...
SCREENS[3] = function(){
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.5rem">Cuvinte cu B</h1>' +
    '<div class="cards">'+cards+'</div>' + nextBtn()
  );
  stage.querySelectorAll('.card').forEach(el=>{ el.onclick = ()=>{ el.classList.add('tapped'); play(B[el.dataset.k].audioArt||B[el.dataset.k].audio); }; });
  wireNext(4);
};
...
SCREENS[4] = function(){
  const READ = [
    {t:'Ball', a:'b-art-ball'}, {t:'Ben', a:'b-ben'}, {t:'Nina', a:'b-nina'},
    {t:'Oma', a:'b-oma'}, {t:'Uli', a:'b-uli'}, {t:'Tom', a:'b-tom'}
  ];
  const SENT = [
    {t:'Ben baut.', a:'st-benbaut'}, {t:'Uli baut.', a:'st-ulibaut'}
  ];
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.1rem">Cu literele învățate + B</h1>' +
...
SCREENS[5] = function(){
  const LEFT  = ['No','Ni','Mo','O'];
  const RIGHT = ['ra','na','ma'];
  const VALID = {
    'No|ra': {w:'Nora', a:'b-nora'},
    'Ni|na': {w:'Nina', a:'b-nina'},
    'Mo|na': {w:'Mona', a:'b-mona'},
    'O|ma':  {w:'Oma',  a:'b-oma'}
  };
...
/* 6 — DESPĂRȚIRE ÎN SILABE (clap + audio pe silabă) */
SCREENS[6] = function(){
  let idx = 0;
  function round(){
    const it = SPLIT[idx];
...
/* 7 — JOC: beginnt mit B? */
SCREENS[7] = function(){
  let tiles = spreadShuffle(GAME_POOL, k=>B[k].m, 2).map(k=>
    '<div class="tile" data-k="'+k+'" style="position:relative"><img src="'+ICON+B[k].icon+'.svg" alt="'+B[k].de+'"></div>'
  ).join('');
  show(
    hero('small') +
    '<p class="game-q">Atinge ce începe cu <span class="m-hi">B</span>!</p>' +
    '<div class="game-grid">'+tiles+'</div>' +
    '<p class="parent-note">Welches Bild beginnt mit B?</p>'
  );
  let found = 0; const need = GAME_POOL.filter(k=>B[k].m).length;
  stage.querySelectorAll('.tile').forEach(el=>{
    el.onclick = function(){
      const w = B[el.dataset.k];
  ...
  function card(){
    ...
    show(
      ...
      '<div class="flashnav">' +
        ...
        '<span class="counter">'+(i+1)+' / '+WORDS_B.length+'</span>' +
    ...
    pv.disabled = (i<=0); nx.disabled = (i>=WORDS_B.length-1);
    pv.onclick = ()=>{ if(i>0){ i--; card(); } };
    nx.onclick = ()=>{ if(i<WORDS_B.length-1){ i++; card(); } };
    wireNext(12);
  }
  card();
};

/* 12 — TRASEAZA B / b — cu SĂGEȚI */
SCREENS[12] = function(){
  let glyph = 'B';
  const STROKE = {'B':'stroke-big','b':'stroke-small'};
  function render(){
    show(
      '<p class="kicker">Schreiben · ① urmează săgețile verzi ✏️</p>' +
      '<div class="btn-row" style="margin-bottom:2px">' +
        '<button class="big-btn'+(glyph==='B'?'':' amber')+'" id="gM">B mare</button>' +
        '<button class="big-btn'+(glyph==='b'?'':' amber')+'" id="gm">b mic</button></div>' +
    ...
    document.getElementById('gM').onclick=()=>{glyph='B';render();};
    document.getElementById('gm').onclick=()=>{glyph='b';render();};
...
/* 14 — SCRIE PROPOZIȚIA */
SCREENS[14] = function(){
  const GLYPH = {'N':'s-cap-n','S':'s-cap-s','R':'s-cap-r','L':'s-cap-l','M':'s-cap-m','T':'s-cap-t','O':'s-cap-o','E':'s-cap-e','F':'s-cap-f','U':'s-cap-u','B':'s-cap-b',
                 'n':'s-n','s':'s-s','r':'s-r','t':'s-t','m':'s-m','a':'s-a','i':'s-i','l':'s-l','o':'s-o','e':'s-e','u':'s-u','b':'s-b'};
  const SENT = [
    {t:'Ben baut.', a:'st-benbaut'},
    {t:'Uli baut.', a:'st-ulibaut'}
  ];
  ...
  function q(){
    ...
    if (it.type==='audio'){
      body = '<p class="game-q">Ascultă 🔊 și atinge imaginea</p>' +
        ...
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+B[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else if (it.type==='findm'){
      body = '<p class="game-q">Care începe cu <span class="m-hi">B</span>?</p>' +
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+B[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else {
      const w = B[it.word];
    ...
    if (it.type==='audio'){
      ...
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (el.dataset.k === it.ans){ el.classList.add('good'); play(B[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else if (it.type==='findm'){
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (B[el.dataset.k].m){ el.classList.add('good'); play(B[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else {
      const w = B[it.word]; play(w.audio);
...
SCREENS[16] = function(){
  show(
    ...
    '<div class="story" style="max-width:380px"><div class="story-row read" id="rima">' +
      '<img src="'+ICON+'baer.svg" alt="">' +
      '<div><div class="s-de"><b class="m-bold">B</b> wie <b class="m-bold">Bär</b>.<br><b class="m-bold">Herzchen</b> umarmt den <b class="m-bold">Bären</b>.</div>' +
      '<div class="s-ro">B ca în Bär. Herzchen îmbrățișează ursul.</div></div>' +
...
function printDiploma(){
  ...
  document.getElementById('diploma-print').innerHTML =
    '<div class="dip">' +
      ...
      '<div class="dip-letter">B b</div>' +
      '<div class="dip-text">B wie Bär — mit Herzchen! 🧡</div>' +
...
function printColoring(){
  ...
  document.getElementById('coloring-print').innerHTML =
    '<div class="color-page">' +
      ...
      '<h2 class="cp-title">B — Herzchen umarmt den Bären</h2>' +
      '<img src="'+ICON+'herzchen-color.svg" alt="Herzchen umarmt den Bären">' +
      '<div class="cp-sentence">Herzchen umarmt den Bären.</div>' +
...
(function initParent(){
  ...
  if (wl) wl.innerHTML = WORDS_B.map(k=>'<span>'+B[k].de+'</span>').join('');
